...
function renderChart() {
    ...
    for (var key in selectedData) {
        innerHTML += "<div class='aqi-bar " + pageState.nowGraTime + "' style='height:" + selectedData[key] + "px; width: " + 
                     posObj.width +"px; left:" + (posObj.left * i + posObj.offsetLeft) + "px; background-color:" + 
                     colors[Math.floor(Math.random() * 11)] + "'></div>"
        innerHTML += "<div class='aqi-hint' style='bottom: " + (selectedData[key] + 10) + "px; left:" + 
                     getHintLfeft(posObj, i++) + "px'>" + key + "<br/> [AQI]: " + selectedData[key] + "</div>"
    }
...
function initAqiChartData() {
    ...
    for (var key in aqiSourceData) {
        var tempCity = aqiSourceData[key];//得到该城市的 数据

        var keyArr = Object.getOwnPropertyNames(tempCity);  //得到上一个对象的所有属性，返回数组，所有日期  keyArr.length = 91
        
        var tempMonth = keyArr[0].slice(5, 7);//月份  01

        var weekInit = 4, weekCount = 0;
        for (var i = 0; i < keyArr.length; i++, weekInit++) {
            count += tempCity[keyArr[i]];  //数据->周
            mcount += tempCity[keyArr[i]];  //数据->月
            weekCount++;
            if ((weekInit+1) % 7 == 0 || i == keyArr.length - 1 || keyArr[i+1].slice(5, 7) !== tempMonth) {
                ...
                if (i != keyArr.length - 1 && keyArr[i+1].slice(5, 7) !== tempMonth) {  //下个月的时候
                ...
                if (i == keyArr.length - 1 || keyArr[i+1].slice(5, 7) !== tempMonth) {
                    ...
                    var tempDays = keyArr[i].slice(-2); //选择出后两位，依据原顺序
